...
sum(is.na(PlacementInsights))

#Note: stanford_insights 418 NA values. The other Stanford Data sets do not have any NA values

#Removing the NA Values from the stanford_insights dataset
Insights <- na.omit(Insights)
sum(is.na(Insights))

#Grouping by 'CampaignId'
Insights <- Insights %>%
  group_by(CampaignId)
...
#Renaming Stanford_Demographics "campaign_ID' column to 'campaign_iD' for merging purposes
Demographics <- Demographics %>%
  rename(CampaignId = CampaignID)

#Picking columns needed
selected_cols <- Insights %>%
  select(Revenue_1d_click, Revenue_1d_view, Revenue_7d_click, Revenue_7d_view, Revenue_28d_click, Revenue_28d_view, Spend)

#Merging
Demographics <- Demographics %>%
  inner_join(selected_cols, by = "CampaignId")
PlacementInsights <- PlacementInsights %>%
  inner_join(selected_cols, by = "CampaignId")
```
